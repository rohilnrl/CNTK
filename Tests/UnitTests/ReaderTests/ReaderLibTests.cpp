...
BOOST_AUTO_TEST_CASE(CheckSetCurrentCursorForRandomizers)
{
    ...
    auto test = [](SequenceEnumeratorPtr expected, SequenceEnumeratorPtr underTest, size_t epochSize)
    {
        ...
        state[L"minibatchSourcePosition"] = firstEpoch.size();
        ...
        state[L"minibatchSourcePosition"] = 0;
        underTest->SetState(state);
        ...
        state[L"minibatchSourcePosition"] = firstEpoch.size() + secondEpoch.size();
        underTest->SetState(state);
...
BOOST_AUTO_TEST_CASE(CheckGetCurrentCursorForRandomizers)
{
    ...
    auto test = [](SequenceEnumeratorPtr r, size_t epochSize)
    {
        ...
        BOOST_CHECK_EQUAL(firstCursor[L"minibatchSourcePosition"], firstEpoch.size());

        auto secondEpoch = ReadFullEpoch(r, epochSize, 1);
        auto secondCursor = r->GetState();
        BOOST_CHECK_EQUAL(secondCursor[L"minibatchSourcePosition"] - firstCursor[L"minibatchSourcePosition"], secondEpoch.size());

        auto thirdEpoch = ReadFullEpoch(r, epochSize, 2);
        auto thirdCursor = r->GetState();
        BOOST_CHECK_EQUAL(thirdCursor[L"minibatchSourcePosition"] - secondCursor[L"minibatchSourcePosition"], thirdEpoch.size());

        auto anotherSecondEpoch = ReadFullEpoch(r, epochSize, 1);
        auto anotherSecondCursor = r->GetState();

        BOOST_CHECK_EQUAL(anotherSecondCursor[L"minibatchSourcePosition"], secondCursor[L"minibatchSourcePosition"]);
    };
...
BOOST_AUTO_TEST_CASE(TestTruncatedBpttPacker)
{
    ...
    std::map<std::wstring, size_t> state;
    state[L"minibatchSourcePosition"] = sweepNumberOfSamples;
